...
普通对话保守追加，不逐轮生成摘要。每次模型请求前检查完整输入预算，仅在超过安全输入上限时先整理再继续；另在连续空闲 20 分钟后检查，超过安全输入预算的 50% 才后台整理。用户不需要新建会话，也没有额外的“待整理”状态。

安全输入预算从模型窗口中扣除输出预留与 10% 余量，整理目标为该预算的 40%。笔记带 `[message:ID]` 原文引用，和较小窗口一起原子保存；保留近期完整工具交互并尽量保留当前用户原话。Token 使用字符估算，非 ASCII 文本采用更保守的比例，并为图片预留用量；这不是各模型的精确 tokenizer。

新输入进入队列时就取消空闲整理，尚未提交的结果丢弃；长期等待事项不阻止整理，正在执行的 Agent 会让整理让路。空闲失败不立即重试，至少等下一完整 20 分钟周期；必要整理最多首次加三次重试，取消、配置错误和明确不可重试的错误立即停止，全部失败则保留原文和步骤并暂停请求。整理请求关闭 SDK 隐式重试，重试仍受运行总超时约束。

切换小窗口模型后，历史按整理模型自己的容量分批读取，再按目标模型预算保留窗口，不要求旧模型仍存在。被移除模型的整理／反思用途引用恢复继承默认模型；整理期间配置变化则放弃旧条件下的提交。
