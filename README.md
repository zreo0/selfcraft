...
当前模型开启 `vision` 时直接接收原图；纯文本模型通过 `image_analyze` 按具体问题调用已配置且凭证可用的视觉模型。辅助读取优先使用当前视觉模型，否则按渠道与模型 ID 的稳定顺序选择，不修改默认模型；配置的视觉模型可能因此收到图片和当前分析问题。没有视觉模型时保留附件并说明无法理解，不阻断后续文字对话。视频、音频和文档输入暂不开放，CLI 暂时保留文字交互。
